...
- **Shows** instant system health with `sysmon status` — color-coded gauges, grouped memory usage, and actionable recommendations
...
╰──────────────────────────────────────────────────────────────╯
╭────────────────────── Recommendations ───────────────────────╮
│  Close browser tabs — browsers using 3.2 GB                  │
│  Reduce AI workspaces — 30 agents active, suggest max 2      │
...
```
sysmon/
├── cli.py           # Click CLI — status, report, collect, install
...
56 tests covering process categorization, database operations (WAL mode, concurrency, rollup, pruning), and collector logic (memory pressure parsing, cmdline hashing).
